#ifndef PHYCORE_VECTOR4_H_
#define PHYCORE_VECTOR4_H_
...
namespace phycore
{
	namespace math
	{
		...
		template<typename T>
		inline T sqr_distance(const detail::Vector4<T>& v1, const detail::Vector4<T>& v2) noexcept
		{
			return length2(v1 - v2);
		}

		template <typename T>
		inline T min(const detail::Vector4<T>& v) noexcept
		{
			return std::min(std::min(std::min(v.x, v.y), v.z), v.w);
		}

		template <typename T>
		inline T max(const detail::Vector4<T>& v) noexcept
		{
